...
MAX_ENTRIES = 12000
OLU_SAYILIR = 3  # bu kadar tur üst üste hata veren kaynak devre dışı kalır
...
def devre_disi_kaynaklar() -> set[str]:
    """Üst üste OLU_SAYILIR kez hata veren kaynakların adları.

    Bu kaynaklar taranmaz: her turda aynı hatayı üretip log'u ve
    Telegram'ı kirletmelerinin anlamı yok. Kaynağın adresi düzeltilip
    tekrar çalışınca sayaç sıfırlanır (--test-sources hepsini dener).
    """
    return {ad for ad, k in _kaynaklari_oku().items()
            if k.get("bos", 0) >= OLU_SAYILIR}


def saglik_guncelle(saglik: list[tuple[str, str]]) -> list[str]:
    """Kaynak sağlığını günceller.

    Döner: bu turda YENİ devre dışı kalan kaynakların listesi
    (sadece bir kez bildirilsin diye).
    """
    veri = _kaynaklari_oku()
    yeni_kapananlar = []
    ...
    for ad, durum in saglik:
        ...
        if kayit["bos"] == OLU_SAYILIR:
            yeni_kapananlar.append(f"• {ad} — {durum[:70]}")

    _kaynaklari_yaz(veri)
    return yeni_kapananlar
